...
export async function registerDoctor(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // ─── 3. Facility Association Validation ───────────────────────────────────
    ...
    if (!facility) {
      throw new AppError(`Selected healthcare facility '${input.facilityId}' not found. Please choose an authorized PHC or CHC.`, 400);
    }
    ...
    // ─── 4. Database Transaction: User + Doctor + HPR Link + Audit ────────────
...
export async function getDoctors(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const dutyStatus = typeof req.query.dutyStatus === 'string' ? req.query.dutyStatus.toUpperCase() : undefined;
...
export async function updateDutyStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const doctor = await prisma.doctor.update({
      where: { id },
      data: { dutyStatus: normalizedStatus },
